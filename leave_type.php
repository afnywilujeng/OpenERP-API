...
class OpenERPXmlrpc {
   ...
   function read($post = null) {
      ...
      if(empty($post)) {
         ...
         $key = array(new xmlrpcval('name','string')  ,new xmlrpcval('limit','string'));
         $this->msg = new xmlrpcmsg('execute');
         $this->msg->addParam(new xmlrpcval($this->database, "string"));
         $this->msg->addParam(new xmlrpcval(1, "int"));
         $this->msg->addParam(new xmlrpcval($this->password, "string"));
         $this->msg->addParam(new xmlrpcval("hr.holidays.status","string"));
         ...
         if(!$this->res->faultCode()) {
          'include"css.php"';         
            $read_html = '<table width="95%" border="0" cellpadding="4" cellspacing="4" align="center">
                        <tr>
                           <th>Leave Type</th>
