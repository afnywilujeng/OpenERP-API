...
class OpenERPXmlrpc {
   ...
   function read($post = null) {
      ...
      if(empty($post)) {
         ...
         $ids_read = array(new xmlrpcval('1', 'int') , new xmlrpcval('2', 'int'), new xmlrpcval('3', 'int'), new xmlrpcval('4', 'int') );
         $key = array(new xmlrpcval('name','string')  , new xmlrpcval('department_id','string')  ,new xmlrpcval('expected_employees','string') , new xmlrpcval('no_of_employee', 'string'),new xmlrpcval('no_of_recruitment','string'),new xmlrpcval('state','string'));
         $this->msg = new xmlrpcmsg('execute');
         $this->msg->addParam(new xmlrpcval($this->database, "string"));
         $this->msg->addParam(new xmlrpcval(1, "int"));
         $this->msg->addParam(new xmlrpcval($this->password, "string"));
         $this->msg->addParam(new xmlrpcval("hr.job","string"));
         ...
         if(!$this->res->faultCode()) {
          'include"css.php"';
            $read_html = '<table width="90%" border="0" cellpadding="4" cellspacing="4" align="center">
                        <tr>
                           <th>Name Job</th>
                           <th>Departement</th>
                           <th>Total forecasted employees</th>
                           <th>current number of employee</th>
                           <th>Expected in Recruitment</th>
                           <th>Status</th>
                        </tr>';
            ...
            foreach ($scalval as $keys => $values) {
               ...
               $read_html .= '
                     <tr align="center">
                        ...
                        <td>'.$value['department_id']->scalarval()[1]->scalarval().'</td>
                        <td>'.$value['expected_employees']->scalarval().'</td>
                        <td>'.$value['no_of_employee']->scalarval().'</td>
                        <td>'.$value['no_of_recruitment']->scalarval().'</td>
                         <td>'.$value['state']->scalarval().'</td>
                     </tr>';
            }
            $read_html .= '</table>';
            return $read_html;
